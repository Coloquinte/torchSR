...
class RealSRv3(FolderByDir):
    ...
    urls = [
        (
            "https://drive.google.com/uc?export=download&id=17ZMjo-zwFouxnm_aFM6CUHBwgRrLZqIM",
            "83e946bf4b5c22b30d1663231bb2e53e",
            "RealSR(V3).tar.gz",
        )
    ]
    ...
    def list_samples_realsr(self, track, split, scale):
        ...
        hr_samples = [
            s for s in all_samples if os.path.splitext(s)[0].lower().endswith("_hr")
        ]
        lr_samples = [
            s for s in all_samples if not os.path.splitext(s)[0].lower().endswith("_hr")
        ]
        if len(lr_samples) != len(hr_samples):
            raise ValueError(
                f"Number of files for {track}X{scale} {split} does not match between HR and LR"
            )
    ...
    def init_samples(self):
        if len(self.tracks) != 1:
            raise RuntimeError(
                "Only one scale can be used at a time for RealSR dataset"
            )
        hr_samples, lr_samples = self.list_samples_realsr(
            self.tracks[0], self.split, self.scales[0]
        )
